...
public abstract class ReusableObjectPool<T extends Reusable> extends BasePooledObjectFactory<T> {
	...
	public T getObject(final String id) {
		...
		try {
			...
			logger.error("Exception when borrow cache object with max " + pool.getMaxTotal() + " and current borrowed " + pool.getBorrowedCount() + " and total create count " + pool.getCreatedCount() + " and total destory count " + pool.getDestroyedCount() + " and idle object " + pool.getNumIdle() + " and mean active time mills " + pool.getMeanActiveTimeMillis() + " MeanBorrowWaitTimeMillis " + pool.getMeanBorrowWaitTimeMillis(), e);
			return null;
		}
	}

	public String getObjectPoolStatus() {
		return "Cache object with max " + pool.getMaxTotal() + ", current borrowed " + pool.getBorrowedCount() + ", total create count " + pool.getCreatedCount() + ", total destory count " + pool.getDestroyedCount() + ", idle object " + pool.getNumIdle() + ", mean active time mills " + pool.getMeanActiveTimeMillis() + ", MeanBorrowWaitTimeMillis " + pool.getMeanBorrowWaitTimeMillis();
	}
