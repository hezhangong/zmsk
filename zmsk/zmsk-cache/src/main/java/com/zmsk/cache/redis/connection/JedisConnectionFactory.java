package com.zmsk.cache.redis.connection;
...
public class JedisConnectionFactory {
	...
	private void init() {
		...
		if (password == null || password.length() == 0) {
			pool = new JedisPool(config, hostname, port, timeout, null, 13); //new JedisPool(config, hostname, port);
			return;
		}
		pool = new JedisPool(config, hostname, port, timeout, password, 13); //new JedisPool(config, hostname, port, timeout, password);
