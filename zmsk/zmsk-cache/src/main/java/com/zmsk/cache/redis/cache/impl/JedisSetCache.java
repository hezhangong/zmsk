...
import com.zmsk.cache.redis.SetCache;
...
public class JedisSetCache<T extends Serializable> extends JedisCacheImpl implements SetCache<T> {
	...
	public JedisSetCache(String namePrefix, String cacheName, JedisConnectionFactory connectionFactory, ReusableObjectPool<JedisSetCache<T>> objectPool) {
	...
	public boolean exists(T member) {
		try {
			return functionHandlerWithOp(2, jedis -> jedis.sismember(getCacheFullNameBytes(), objectToBytes(member)), RedisOperation.sismember);
		} finally {
			this.autoRecycle();
		}
	}
	...
	public List<T> getRandomMembers(int size) {
		try {
			return toObjectsList(functionHandlerWithOp(2, jedis -> jedis.srandmember(getCacheFullNameBytes(), size), RedisOperation.srandmembers));
		} finally {
			this.autoRecycle();
		}
	}
	...
	@Override
	public void addToSetExpire(int seconds,@SuppressWarnings("unchecked") T... members) {
		if (members == null || members.length == 0) {
			return ;
		}
		try {
			functionHandlerWithOp(2, jedis -> jedis.sadd(getCacheFullNameBytes(), arrayToBytes(members)), RedisOperation.sadd);
			functionHandlerWithOp(2, jedis -> jedis.expire(getCacheFullName(), seconds), RedisOperation.expire);
		} finally {
			this.autoRecycle();
		}
	}
