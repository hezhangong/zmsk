...
import com.zmsk.cache.redis.SortedSetCache;
...
public class JedisSortedSetCache<T extends Serializable> extends JedisCacheImpl implements SortedSetCache<T> {
	...
	public JedisSortedSetCache(String namePrefix, String cacheName, JedisConnectionFactory connectionFactory, ReusableObjectPool<JedisSortedSetCache<T>> objectPool) {
	...
	public double zincrby(double increment, T member) {
		try {
			return functionHandlerWithOp(2, jedis -> jedis.zincrby(getCacheFullNameBytes(), increment, objectToBytes(member)), RedisOperation.zincrby);
		} finally {
			this.autoRecycle();
		}
	}
	...
	public Set<Tuple> zrangeWithScores(long start, long end) {
		try {
			return functionHandlerWithOp(2, jedis -> jedis.zrangeWithScores(getCacheFullName(), start, end), RedisOperation.zrangeWithScore);
		} finally {
			this.autoRecycle();
		}
	}
	...
	public Set<Tuple> zrevrangeWithScores(long start, long end) {
		try {
			return functionHandlerWithOp(2, jedis -> jedis.zrevrangeWithScores(getCacheFullName(), start, end), RedisOperation.zrevrangeWithScores);
		} finally {
			this.autoRecycle();
		}
	}
	...
	public Set<T> zrangeByScore(double min, double max) {
		try {
			return toObjectsSet(functionHandlerWithOp(2, jedis -> jedis.zrangeByScore(getCacheFullNameBytes(), min, max), RedisOperation.zrangeByScore));
		} finally {
			this.autoRecycle();
		}
	}
	...
	public Set<Tuple> zrangeByScoreWithScores(final double min, final double max) {
		try {
			return functionHandlerWithOp(2, jedis -> jedis.zrangeByScoreWithScores(getCacheFullName(), min, max), RedisOperation.zrangeByScoreWithScores);
		} finally {
			this.autoRecycle();
		}
	}
	...
	public long zremrangebyrank(long start, long end) {
		try {
			return functionHandlerWithOp(2, jedis -> jedis.zremrangeByRank(getCacheFullNameBytes(), start, end), RedisOperation.zremrangeByRank);
		} finally {
			this.autoRecycle();
		}
	}
	...
	public long zremrangebysore(double min, double max) {
		try {
			return functionHandlerWithOp(2, jedis -> jedis.zremrangeByScore(getCacheFullNameBytes(), min, max), RedisOperation.zremrangeByScore);
		} finally {
			this.autoRecycle();
		}
	}
