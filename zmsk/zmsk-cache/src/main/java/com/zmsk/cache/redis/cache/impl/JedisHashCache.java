...
import com.zmsk.cache.redis.HashCache;
...
public class JedisHashCache<K extends Serializable, V extends Serializable> extends JedisCacheImpl implements HashCache<K, V> {
	...
	public JedisHashCache(final String namePrefix, final String cacheName, final JedisConnectionFactory connectionFactory, final ReusableObjectPool<JedisHashCache<K, V>> objectPool) {
	...
	@Override
	public void putAllEx(Map<K, V> map, int seconds) {
		if (map == null || map.size() == 0) {
			return;
		}
		try {
			functionHandlerWithOp(2, jedis -> jedis.hmset(getCacheFullNameBytes(), mapToBytes(map)), RedisOperation.hmset).equals(OK);
	...
	public Set<String> getCacheKeys() {
		try {
			Set<String> keys = functionHandlerWithOp(2, jedis -> jedis.keys(getCacheFullName()), RedisOperation.keys);
			return keys;
		} finally {
			this.autoRecycle();
		}
