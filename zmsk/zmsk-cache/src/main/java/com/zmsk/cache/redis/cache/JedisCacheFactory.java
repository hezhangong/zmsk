...
import com.zmsk.cache.exception.DuplicateCacheDeclareException;
import com.zmsk.cache.redis.cache.impl.JedisHashCache;
import com.zmsk.cache.redis.cache.impl.JedisListCache;
import com.zmsk.cache.redis.cache.impl.JedisNumbericHashCache;
import com.zmsk.cache.redis.cache.impl.JedisSetCache;
import com.zmsk.cache.redis.cache.impl.JedisSortedSetCache;
import com.zmsk.cache.redis.cache.impl.JedisStringCache;
	...
	public <T extends JedisCacheImpl> JedisCacheGroup<? extends JedisCacheImpl> createCache(final String cacheName, final Class<T> cacheClass, int maxTotal, int maxIdle, int minIdle) {
		...
		synchronized (cacheName.intern()) {
			...
			try {
				...
				logger.error(e.getClass().getSimpleName() + " when create cache with name {}", cacheName, e);
			}
			return cache;
		}
	}
